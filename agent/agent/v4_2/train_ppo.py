"""
V4.2 PPO training: V4.1 architecture with stage-aware actor warm-ups.
"""
...
from model import CNNActorCriticV4_2, MASK_WARMUP_STEPS, prepare_policy_inputs, to_env_action
...
class ActiveOpponentPoolV4_2(base.ActiveOpponentPool):
    def _load_checkpoint_opponent(self, checkpoint, agent_id: int, current_step: int):
        model = self.model_factory()
        model.load_state_dict(checkpoint)
        model.to(DEVICE)
        model.eval()
        return ModelOpponentV4_2(model, agent_id, current_step)


class ModelOpponentV4_2:
...
def train():
    ...
    model = CNNActorCriticV4_2(num_actions=6).to(DEVICE)
    ...
    pool = ActiveOpponentPoolV4_2(
        model_factory=lambda: CNNActorCriticV4_2(num_actions=6),
    ...
    print(f"\n{'=' * 60}")
    print("Training PPO Agent V4.2 - Stage-Aware Warm-Up")
    print(
    ...
    while global_step < CFG["total_steps"]:
        ...
        if global_step - last_save_step >= CFG["save_every"]:
            ...
            if dev_stats["score"] >= best_eval_score:
                ...
                print(f"  -> New best v4.2 model! Dev score: {best_eval_score:.3f}")
