...
import train_ppo_v3 as base
from model_v4_1 import CNNActorCriticV4_1, MASK_WARMUP_STEPS, prepare_policy_inputs, to_env_action
...
CFG.update(
    {
        ...
        "actor_init_path": str(_HERE / "model_v3.pth"),
        "ckpt_dir": "checkpoints_v4_1",
    }
)
...
def train():
    ...
    while global_step < CFG["total_steps"]:
        ...
        if global_step - last_save_step >= CFG["save_every"]:
            ...
            ckpt_path = Path(CFG["ckpt_dir"]) / f"model_v4_1_step{global_step}.pth"
            torch.save(
                {
            ...
            if dev_stats["score"] >= best_eval_score:
                ...
                torch.save(
                    ...
                    _HERE / "model_v4_1.pth",
                )
                print(f"  -> New best v4.1 model! Dev score: {best_eval_score:.3f}")

    last_model_path = _HERE / "model_v4_1_last.pth"
    torch.save(
        {
            "model": model.state_dict(),
            "global_step": global_step,
    ...
    print(f"\nTraining done! Best eval model saved to {_HERE / 'model_v4_1.pth'}")
