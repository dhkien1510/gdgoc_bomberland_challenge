"""
V5.1 PPO training: V5 reward shaping with tactical features and bomb-quality logs.
"""
...
from model import CNNActorCriticV5_1, MASK_WARMUP_STEPS, prepare_policy_inputs, to_env_action
...
class ActiveOpponentPoolV5_1(base.ActiveOpponentPool):
    def _load_checkpoint_opponent(self, checkpoint, agent_id: int, current_step: int):
        model = self.model_factory()
        model.load_state_dict(checkpoint)
        model.to(DEVICE)
        model.eval()
        return ModelOpponentV5_1(model, agent_id, current_step)


class ModelOpponentV5_1:
...
def train():
    ...
    model = CNNActorCriticV5_1(num_actions=6).to(DEVICE)
    ...
    pool = ActiveOpponentPoolV5_1(
        model_factory=lambda: CNNActorCriticV5_1(num_actions=6),
    ...
    print(f"\n{'=' * 60}")
    print("Training PPO Agent V5.1 - Tactical Features + Valuable Bomb Mask")
    print(
    ...
    while global_step < CFG["total_steps"]:
        ...
        if global_step - last_save_step >= CFG["save_every"]:
            ...
            if dev_stats["score"] >= best_eval_score:
                ...
                print(f"  -> New best v5.1 model! Dev score: {best_eval_score:.3f}")
