...
def enemy_same_row_or_col_with_clear_path(obs: dict, agent_id: int) -> bool:
    ...
    for i, player in enumerate(players):
        ...
        if row == my_row:
            lo, hi = sorted((my_col, col))
            blocked = any(int(grid[row, c]) in (1, 2) for c in range(lo + 1, hi))
            if not blocked:
                return True
        if col == my_col:
            lo, hi = sorted((my_row, row))
            blocked = any(int(grid[r, col]) in (1, 2) for r in range(lo + 1, hi))
            if not blocked:
                return True
    return False
...
def valid_action_mask(
    ...
    if bombs_left > 0 and (row, col) not in bomb_tiles and safe_at_time((row, col), 1):
        ...
        strict_value_bomb_mask = eval_mode or current_step >= value_bomb_mask_steps
