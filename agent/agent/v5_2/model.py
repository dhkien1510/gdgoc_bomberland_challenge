"""
V5.2 actor/critic-separated model for Bomberland PPO.
"""
...
class CNNActorCriticV5_2(nn.Module):
    """
    ...
    def load_actor_from_checkpoint(self, checkpoint: dict | str):
        ...
        for key, value in state_dict.items():
            ...
            if key.startswith("cnn."):
                new_key = "actor_cnn." + key[len("cnn."):]
            elif key.startswith("shared_fc."):
                new_key = "actor_fc." + key[len("shared_fc."):]
        ...
        loaded_actor_cnn = any(key.startswith("actor_cnn.") for key in remapped)
        loaded_actor_fc0 = any(key.startswith("actor_fc.0.") for key in remapped)
        loaded_actor = loaded_actor_cnn and loaded_actor_fc0
        return {
