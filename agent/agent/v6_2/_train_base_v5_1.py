...
CURRICULUM_STAGES = [
    {
        ...
        "reward": {
            ...
            "r_danger_critical": -0.10,
            "r_danger_soon": -0.04,
            "r_danger_far": -0.01,
            "r_escape_danger": 0.015,
            "r_valuable_bomb_enemy": 0.10,
            "r_valuable_bomb_box": 0.05,
            "r_useless_bomb": -0.08,
            "r_no_escape_bomb": -0.30,
    ...
    },
    {
        ...
        "reward": {
            ...
            "r_move_closer": 0.001,
            "r_move_away": -0.001,
            "r_best_enemy_dist": 0.02,
            "r_danger_critical": -0.16,
            "r_danger_soon": -0.06,
            "r_danger_far": -0.015,
    ...
    },
    {
        ...
        "reward": {
            ...
            "rank_rewards": {0: 1.0, 1: 0.2, 2: -0.3, 3: -1.0},
        },
    },
    {
        "name": "phase4_full",
        ...
        "reward": {
            ...
            "r_move_closer": 0.001,
            "r_move_away": -0.001,
            "r_best_enemy_dist": 0.01,
            "r_danger_critical": -0.24,
            "r_danger_soon": -0.10,
            "r_danger_far": -0.025,
            "r_escape_danger": 0.04,
            "r_valuable_bomb_enemy": 0.30,
            "r_valuable_bomb_box": 0.04,
            "r_useless_bomb": -0.12,
            "r_no_escape_bomb": -0.60,
            "rank_rewards": {0: 2.0, 1: 0.4, 2: -0.5, 3: -2.0},
        },
    },
]
...
def summarize_episode_metrics(episode_ctx: dict, final_stats: dict) -> dict:
    bombs_placed = int(episode_ctx["bombs_placed"])
    if bombs_placed > 0:
        valuable_ratio = float(episode_ctx["valuable_bombs"]) / bombs_placed
        useless_ratio = float(episode_ctx["useless_bombs"]) / bombs_placed
        no_escape_ratio = float(episode_ctx["no_escape_bombs"]) / bombs_placed
    else:
        valuable_ratio = 0.0
        useless_ratio = 0.0
        no_escape_ratio = 0.0

    unique_tiles = len(episode_ctx["visited_tiles"])
    total_steps = max(unique_tiles + int(episode_ctx["repeat_steps"]) - 1, 1)
    repeat_position_rate = float(episode_ctx["repeat_steps"]) / total_steps

    return {
        "bombs_per_episode": float(bombs_placed),
        "valuable_bomb_ratio": valuable_ratio,
        "useless_bomb_ratio": useless_ratio,
        "no_escape_bomb_ratio": no_escape_ratio,
        "danger_steps_per_episode": float(episode_ctx["danger_steps"]),
        "kills_per_episode": float(final_stats["kills"]),
        "boxes_per_episode": float(final_stats["boxes"]),
        "items_per_episode": float(final_stats["items"]),
        "unique_tiles_visited": float(unique_tiles),
        "repeat_position_rate": repeat_position_rate,
    }
