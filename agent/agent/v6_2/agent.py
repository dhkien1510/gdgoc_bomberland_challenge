...
class Agent:
    def __init__(self, agent_id: int):
        ...
        bc_candidates = [
            _HERE / "bc_actor.pth",
            _HERE / "bc_actor_public_top1.pth",
            _HERE / "bc_actor_public_top1_finetune.pth",
        ]
        ...
        if model_path is not None:
            ...
            if bc_path is None:
                candidate_text = ", ".join(str(path) for path in [*model_candidates, *bc_candidates])
                raise FileNotFoundError(f"No usable checkpoint found in [{candidate_text}]")
            bc_state = _load_checkpoint(bc_path, map_location=self.device)
            state_dict = bc_state.get("model", bc_state) if isinstance(bc_state, dict) else bc_state
            try:
                self.model.load_state_dict(state_dict)
            except RuntimeError:
                _load_state_dict_expand_input(self.model, state_dict, "bc")
