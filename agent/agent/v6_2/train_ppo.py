...
_TRAIN_BASE_SPEC = importlib.util.spec_from_file_location("_v6_2_train_base_train_ppo", _HERE / "_train_base.py")
...
_MODEL_SPEC = importlib.util.spec_from_file_location("_v6_2_model_train_ppo", _HERE / "model.py")
...
CFG.update(
    {
        ...
        "ppo_epochs": 4,
        "bc_coef": 0.05,
        "actor_core_lr": 3.0e-5,
        "actor_head_lr": 7.5e-5,
        "critic_lr": base.CFG["lr"],
        "initial_actor_warmup_steps": 25_000,
        "stage_actor_warmup_steps": 5_000,
        "save_every": 50_000,
        "eval_easy_medium_matches": 10,
        "eval_hard_matches": 10,
        "actor_init_candidates": [
            str(_HERE / "bc_actor_public_top1_finetune.pth"),
            str(_HERE / "bc_actor_public_top1.pth"),
...
def parse_args():
    ...
    parser.add_argument("--bc_reference_path", type=str, default=str(_HERE / "bc_actor_public_top1.pth"))
...
def train():
    ...
    if resume_checkpoint_path is not None:
        ...
        try:
            ...
            print(f"Strict load failed; trying expanded-input load for v6_2: {exc}")
    ...
    print(f"\n{'=' * 60}")
    print("Training Recurrent PPO Agent V6_2 - BC Initialized")
    print(
    ...
    while global_step < CFG["total_steps"]:
        ...
        if global_step - last_save_step >= CFG["save_every"]:
            ...
            if eval_stats["score"] >= best_eval_score:
                ...
                print(f"  -> New best v6_2 model! Eval score: {best_eval_score:.3f}")
