...
def train():
    ...
    while global_step < CFG["total_steps"]:
        ...
        if global_step - last_save_step >= CFG["save_every"]:
            ...
            ckpt_path = Path(CFG["ckpt_dir"]) / f"model_v2_step{global_step}.pth"
            torch.save(
                {
    ...
    last_model_path = _HERE / "model_v2_last.pth"
    torch.save(
        {
            "model": model.state_dict(),
            "global_step": global_step,
    ...
    if best_eval_score == float("-inf"):
        torch.save({"model": model.state_dict(), "global_step": global_step}, _HERE / "model_v2.pth")

    print(f"\nTraining done! Best eval model saved to {_HERE / 'model_v2.pth'}")
