...
def _virtual_bomb_info(obs: dict, agent_id: int) -> tuple[set[tuple[int, int]], int]:
    ...
    if virtual is None:
        return set(), 0
    return set(virtual["blast_tiles"]), int(virtual["radius"])


def count_boxes_if_place(obs: dict, agent_id: int) -> int:
    blast_tiles, _ = _virtual_bomb_info(obs, agent_id)
    if not blast_tiles:
        return 0
    grid = np.asarray(obs["map"], dtype=np.int64)
    return sum(1 for row, col in blast_tiles if int(grid[row, col]) == 2)
...
def can_hit_enemy_if_place(obs: dict, agent_id: int) -> bool:
    ...
    players = np.asarray(obs["players"], dtype=np.int64)
    for i, player in enumerate(players):
        if i == agent_id or int(player[2]) != 1:
            continue
        if (int(player[0]), int(player[1])) in blast_tiles:
            return True
    return False


def has_attack_pressure(obs: dict, agent_id: int) -> bool:
    dist = nearest_enemy_distance(obs, agent_id)
    return (0.0 < dist <= 6.0) or can_hit_enemy_if_place(obs, agent_id)
