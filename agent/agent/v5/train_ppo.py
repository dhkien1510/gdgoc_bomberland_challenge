"""
V5 PPO training: V4.2 architecture with reward shaping focused on tactical bombs.
"""
...
from model import CNNActorCriticV5, MASK_WARMUP_STEPS, prepare_policy_inputs, to_env_action
...
class ActiveOpponentPoolV5(base.ActiveOpponentPool):
    def _load_checkpoint_opponent(self, checkpoint, agent_id: int, current_step: int):
        model = self.model_factory()
        model.load_state_dict(checkpoint)
        model.to(DEVICE)
        model.eval()
        return ModelOpponentV5(model, agent_id, current_step)


class ModelOpponentV5:
...
def train():
    ...
    model = CNNActorCriticV5(num_actions=6).to(DEVICE)
    ...
    pool = ActiveOpponentPoolV5(
        model_factory=lambda: CNNActorCriticV5(num_actions=6),
    ...
    print(f"\n{'=' * 60}")
    print("Training PPO Agent V5 - Tactical Reward Shaping")
    print(
    ...
    while global_step < CFG["total_steps"]:
        ...
        if global_step - last_save_step >= CFG["save_every"]:
            ...
            if dev_stats["score"] >= best_eval_score:
                ...
                print(f"  -> New best v5 model! Dev score: {best_eval_score:.3f}")
